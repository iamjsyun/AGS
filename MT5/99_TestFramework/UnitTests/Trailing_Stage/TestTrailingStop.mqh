...
public:
    static bool Run() {
        ...
        //--------------------------------------------------------------
        {
            ...
            // 3. Evaluation & Execution (Should NOT trigger at peak)
            int res_eval = tEvaluate.Execute(GetPointer(xp), GetPointer(ctx));
            int res_exec = tExecute.Execute(GetPointer(xp), GetPointer(ctx));

            if(res_exec == TASK_CONTINUE) {
                Print("  [PASS] Scenario 1: No retraction at peak, trailing stop not triggered.");
            } else {
                PrintFormat("  [FAIL] Scenario 1: Trailing stop triggered prematurely at peak. Res: %d", res_exec);
                allPassed = false;
            }

            // 4. Retraction Trigger (Price drops to 2350.00)
            pricer.SetPrice(2350.00); // Retraction of 5.00 >= TSStep (5.00)
            res_eval = tEvaluate.Execute(GetPointer(xp), GetPointer(ctx));
            res_exec = tExecute.Execute(GetPointer(xp), GetPointer(ctx));

            if(res_exec == 20) {
                Print("  [PASS] Scenario 1: Retraction detected, successfully transitioned to liquidation (20).");
            } else {
                PrintFormat("  [FAIL] Scenario 1: Retraction not triggered. Expected 20, got: %d", res_exec);
                allPassed = false;
            }
        }
