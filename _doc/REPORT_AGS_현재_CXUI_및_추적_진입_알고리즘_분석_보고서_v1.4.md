# REPORT_AGS_현재_CXUI_및_추적_진입_알고리즘_분석_보고서_v1.4.md

## Document History
- **v1.4** (2026-06-03) 사용자 피드백 반영 (신호 감지 시 시장가 SIG 및 진입 시 TP 가격을 SQLite DB 테이블 컬럼에 영구 업데이트하도록 저장 로직 고도화, EA 재기동 시 데이터 보존 및 UI 출력 일치화 명세 추가)
...
```
- `SIG`: 최초 신호 감지(Discovery) 시점의 시장 가격. (DB의 `price` 컬럼에 업데이트하여 영구 보존)
...
- `TP`: Take Profit 설정 가격 및 진입가 대비 TP까지의 목표 포인트 (`{TP point, 포지션 진입가 + tp point}`). (포지션 진입 시 결정된 TP 가격인 `price_tp` 값을 DB에 동시 업데이트하여 영구 보존)
...
4. **재가동 시의 영속성 및 가격 정보 일치화 (Restart State Preservation)**
   - 포지션 진입 시 결정된 감지 시장가(`SIG`) 및 목표가(`TP`) 등의 실체 변수들을 `IRepository::UpdateStatus()` 시점에 SQLite DB(`price`, `price_tp`, `price_sl`, `price_open` 컬럼)로 동시 갱신합니다.
   - 이를 통해 터미널이나 EA가 갑자기 재가동(Reboot)되어도 테이블로부터 정확한 가격 정보를 복구하여 차트 UI에 안정적으로 출력되도록 보장합니다.
