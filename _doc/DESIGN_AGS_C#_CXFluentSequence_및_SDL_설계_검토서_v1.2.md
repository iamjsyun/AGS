# [Design] C# CXFluentSequence 및 SDL 설계 및 구현 검토서 (v1.2)
...
namespace AGS.Core.Orchestration
{
    ...
    public class CXFluentSequence : ICXFluentSequence
    {
        ...
        protected int _tmpFrom = -1;
        protected IXStage _tmpStage;
        protected IXGuard _tmpGuard;
        protected int _tmpSuccess = -1;
        protected int _tmpFail = -1;
        protected int _tmpTimeout = 0;
        protected int _tmpRetries = 0;
        protected readonly Dictionary<int, int> _tmpBranches = new();
...
namespace AGS.Core.Orchestration.SDL
{
    public class CXSequenceStageConfig
    {
        ...
        public bool IsParallelTasks { get; set; } = false; // [v1.2] 병렬 처리 플래그
    ...
    public class SDLParser
    {
        ...
        public List<CXSequenceStageConfig> ParseSDL(string[] dslLines)
        {
            ...
            foreach (var s in dslLines)
            {
                ...
                // 태스크 파싱 및 병렬 연산자(||) 여부 확인
                if (!string.IsNullOrEmpty(taskStr))
                {
                    string[] tasks;
                    if (taskStr.Contains("||"))
                    {
                        config.IsParallelTasks = true;
                        tasks = taskStr.Split(new[] { "||" }, StringSplitOptions.RemoveEmptyEntries);
                    }
                    else
                    {
                        tasks = taskStr.Split(',');
...
namespace AGS.Core.Orchestration.SDL
{
    public static class SDLCompiler
    {
        public static void Compile(
            ...
            Func<string, string, List<string>, bool, IXStage> stageFactory) // [v1.2] IsParallel 플래그 전달
        {
            ...
            {
                IXStage stage = stageFactory(cfg.StageTypeStr, cfg.Name, cfg.Tasks, cfg.IsParallelTasks);
                if (stage == null)
                {
...
### 6.4. 비동기 병렬 실행 및 조인 대기 기능 (Parallel Execution & Joining Sync)

C#의 `Task.WhenAll` 메커니즘을 결합하여 여러 개의 스테이지 또는 태스크를 병렬로 독립 기동하고, 모든 처리가 완료(Join)된 시점에 집계 결과(성공/실패 여부)를 바탕으로 다음 상태(Sequence)나 태스크로 이행하는 고급 컴포넌트 모델입니다.

#### 6.4.1. CXParallelCompositeStage 설계
여러 비동기 스테이지를 감싸서 병렬 실행 및 결과를 대기하는 복합 스테이지 구현체 명세입니다.
...
{
    /// <summary>
    /// 여러 개의 비동기 스테이지를 동시에(Task.WhenAll) 구동하고 모든 결과를 결합(Join)하여 
    /// 최종 성공/실패 여부에 따라 다음 시퀀스를 전이하는 병렬 비동기 스테이지 컨테이너입니다.
    /// </summary>
    public class CXParallelCompositeStage : IXStageAsync
    {
        public string Name { get; }
        private readonly List<IXStageAsync> _stages = new();

        public CXParallelCompositeStage(string name, IEnumerable<IXStageAsync> stages)
        {
            Name = name;
            _stages.AddRange(stages ?? throw new ArgumentNullException(nameof(stages)));
        }

        public bool OnCondition(ICXParam xp, ICXContext ctx, int currentState)
        {
            // 동기식 대체 처리: 모든 스테이지가 진입 조건(OnCondition)을 충족해야 함
            return _stages.All(s => s.OnCondition(xp, ctx, currentState));
        }

        public async Task<bool> OnConditionAsync(ICXParam xp, ICXContext ctx, int currentState, CancellationToken cancellationToken = default)
        {
            // 모든 스테이지의 진입 조건을 비동기로 병렬 평가
            var tasks = _stages.Select(s => s.OnConditionAsync(xp, ctx, currentState, cancellationToken));
            bool[] results = await Task.WhenAll(tasks);
            return results.All(r => r);
        }

        public int OnProcess(ICXParam xp, ICXContext ctx)
        {
            throw new NotSupportedException("Parallel composite stage must be invoked asynchronously via OnProcessAsync.");
        }

        /// <summary>
        /// 등록된 하위 스테이지들을 병렬(Task.WhenAll) 실행 후 결합(Join)합니다.
        /// </summary>
        public async Task<int> OnProcessAsync(ICXParam xp, ICXContext ctx, CancellationToken cancellationToken = default)
        {
            // 1. 병렬 비동기 가동 (Start all tasks concurrently)
            var tasks = _stages.Select(s => s.OnProcessAsync(xp, ctx, cancellationToken));
            
            // 2. 모든 비동기 태스크 완료 대기 및 조인 (Task Join Barrier)
            int[] results = await Task.WhenAll(tasks);

            // 3. 실행 결과 집계 및 결정 논리
            // 단 하나라도 실패(-99 또는 오류 코드)를 반환하면 대표 에러(-99)로 분기 처리
            if (results.Any(r => r == -99 || r == -1))
            {
                return -99; 
            }

            // 모든 비동기 스테이지가 정상적으로 완료되면 0(STAGE_SUCCESS)을 반환하여 IfTrue 시퀀스로 전환
            return 0;
        }

        public void OnEnter(ICXContext ctx)
        {
            foreach (var stage in _stages) stage.OnEnter(ctx);
        }

        public async Task OnEnterAsync(ICXContext ctx, CancellationToken cancellationToken = default)
        {
            var tasks = _stages.Select(s => s.OnEnterAsync(ctx, cancellationToken));
            await Task.WhenAll(tasks);
        }

        public void OnExit(ICXContext ctx)
        {
            foreach (var stage in _stages) stage.OnExit(ctx);
        }

        public async Task OnExitAsync(ICXContext ctx, CancellationToken cancellationToken = default)
        {
            var tasks = _stages.Select(s => s.OnExitAsync(ctx, cancellationToken));
            await Task.WhenAll(tasks);
        }
    }
}
...
#### 6.4.2. SDL(DSL) 문법 확장 연동 규격
비동기 조인 기능을 파싱 스트림에서 직접 선언할 수 있도록 SDL 문법 규칙을 확장합니다.

- **기존 (순차 복합 태스크)**: `: TASK_A, TASK_B, TASK_C`
- **확장 (병렬 조인 복합 태스크)**: `: TASK_A || TASK_B || TASK_C` (또는 StageType을 `Parallel`로 지정)
- **컴파일 연동**: `SDLParser`가 태스크 정의부 내에서 `||` 연산자를 감지하면 `IsParallelTasks = true` 플래그를 할당하며, `SDLCompiler` 및 팩토리는 해당 목록을 동기적으로 순회하는 `CXCompositeStage` 대신 병렬 기동 및 대기를 수행하는 `CXParallelCompositeStage`로 실체화합니다.
...
| v1.2 | 2026-06-01 | Antigravity | - 사용자 피드백 반영: 다중 비동기 태스크/스테이지 병렬 기동 및 대기 결합(Wait/Join)을 수행하는 `CXParallelCompositeStage` 및 SDL 연동 구문 설계 확장 |
