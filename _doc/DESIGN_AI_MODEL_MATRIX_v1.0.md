# AGS AI Model Execution & Selection Matrix (v1.0)
...
| :--- | :--- | :--- | :--- | :--- |
| **Tier 1** | **Verification & Polling** | `CXTaskActive_V_Terminal`<br>`CXTaskExit_V_Terminal`<br>`CXTaskSync_V_Stale` | - Latency < 100ms<br>- High API frequency<br>- Low-cost scaling | **Gemini 1.5 Flash** |
| **Tier 2** | **Stateful Decisions** | `CXTaskPending_V_Sync`<br>`CXTaskIntentWatch`<br>`CXTaskExit_P_Lock`<br>`CXTaskExit_P_Finalize` | - Strict rules compliance<br>- Reliable state routing<br>- Medium latency | **Gemini 1.5 Pro** |
| **Tier 3** | **Mathematical Evaluation** | `CXTaskTrail_L_Evaluate`<br>`CXTaskTrail_R_Execute`<br>Dynamic SL/TP Fitting | - Analytical deduction<br>- Statistical confidence<br>- Multivariable heuristics | **Gemini 1.5 Pro / Ultra** |
| **Tier 4** | **Agentic Engineering** | TSDL test creation (`Test*.mqh`), PowerShell scripts, code generation | - Perfect code compile (`0 errors`) | **Gemini Advanced / Specialized** |

---

## 3. Tier Specifications & Architectural Rationales

### 3.1 Tier 1: Real-time Verification & Polling (Gemini 1.5 Flash)
...
- **Rationals**:
  ...
  - **High API Footprint**: Continuous polling across multiple active sessions generates substantial API volume. Gemini 1.5 Flash offers the required performance-to-cost ratio.
...
### 3.2 Tier 2: Stateful Logic & Transition Gates (Gemini 1.5 Pro)
...
### 3.3 Tier 3: Mathematical Heuristics (Gemini 1.5 Pro / Ultra)
...
### 3.4 Tier 4: Code Generation and Agentic Build Tools (Gemini Advanced)
