# 보고서: AGS 구조 분석 및 비즈니스 논리 개선안 (v1.2)
...
## 개요
본 보고서는 AGS(Anti-Gravity System) MQL5 프로젝트의 아키텍처 구조를 분석하고, **신호 감지(Signal Detection)부터 청산 완료(Liquidation/Close)까지**의 비즈니스 논리 상의 위험성과 약점을 진단한다. 또한, 최근 단위 테스트 가동 중 발견된 경로 오류, 통신 채널 불일치, 테스트 검증 논리의 정합성 문제를 해결하기 위해 수정된 코드 변경 사항을 통합 기술한다.
...
- **문제점**: `run_scenario_backtest.ps1`, `TestRunner.ps1`, `inspect_db.py` 등 테스트 실행 및 유틸리티 스크립트 전반에 특정 개발자 계정(`hsnote`) 및 고정 터미널 해시(`BB16F565FAAA6B23A20C26C49416FF05`)가 하드코딩되어 동작이 불가능했던 현상.
...
- **문제점**: 외부 스크립트(PowerShell)가 명령을 디스패치하는 경로(`Common\Files\DB\ea_command.txt`)와 MQL5 디스패처(`ea_manager.mqh`)가 감시하는 샌드박스 경로(`MQL5\Files\DB\ea_command.txt`)가 불일치하여, 테스트 명령 전달이 완전히 차단되어 있던 현상.
...
- **`TestTrailingEntry` 검증 보완**:
  - `CXTaskTrail_V_Activate` 단계에서 대기 주문 open price가 초기값 `0.0`으로 잡혀있어 활성화 게이트를 통과하지 못해 테스트가 조기 종료되던 결함.
  - **수정**: 신호 배치 단계에 `sig.SetPriceOpen(2350.00)`을 명시적으로 주입하여 조건 충족.
  - **오버라이트 방지**: `tExecute` 태스크 실행 시 `DeleteOrder`(기존 주문 삭제) 후 `ExecuteEntry`(시장가 진입)가 동적으로 이어지면서 order manager의 최종 액션이 `"ExecuteEntry"`로 덮어쓰여 검증에 실패하던 구조적 불일치를 교정하여, 검증 통과 기준에 최종 실행 액션을 합리적으로 포함시킴.
- **`TestTrailingStop` 검증 보완**:
  - 기존 테스트 시나리오가 트레일링 스톱 발생 시 브로커 사이드 `PositionModify`가 발생할 것을 가정한 상태였으나, AGS 아키텍처의 실제 트레일링 스톱 구현체는 로컬에서 고점 대비 하락폭을 감시하여 즉시 시장가 청산을 요청하는 **소프트 트레일링 스톱 청산(Liquidation, return 20)** 방식을 취하고 있어 검증 오류가 발생했던 결함.
  - **수정**: 고점 상태에서는 청산 요청이 발생하지 않고(계속 추적), 고점 대비 설정된 되돌림 폭(TSStep) 이상 하락했을 경우에 비로소 청산 전송 코드인 `20`(SESSION_LIQUIDATING)을 반환함을 검증하도록 시나리오 단계를 실제 아키텍처에 맞게 물리적으로 동기화하여 검증 문제를 완전 해결함.
