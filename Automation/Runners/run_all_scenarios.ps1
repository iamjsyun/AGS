...
$WorkspaceScripts = [System.IO.Path]::GetFullPath((Join-Path -Path $PSScriptRoot -ChildPath "..\..\Test\01_Scenarios"))
$ManifestPath = [System.IO.Path]::GetFullPath((Join-Path -Path $PSScriptRoot -ChildPath "..\..\Test\01_Scenarios\scenario_manifest.json"))
$ReportPath = [System.IO.Path]::GetFullPath((Join-Path -Path $PSScriptRoot -ChildPath "..\..\_doc\result\test_results_summary.json"))
...
foreach ($scen in $manifest.scenarios) {
    ...
    $configPath = [System.IO.Path]::GetFullPath((Join-Path -Path $PSScriptRoot -ChildPath "..\..\Test\02_Config\runner_startup.ini"))
    $process = Start-Process -FilePath $TerminalPath -ArgumentList "/config:$configPath" -PassThru -NoNewWindow
    if ($process -ne $null) {
